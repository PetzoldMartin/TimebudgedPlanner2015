...
public class BookingQueryToolBean implements BookingQueryTool {
	...
	public BookingOverview getBookingOverview(int bookingId) {
		...
		overview.setId(bookingId);
		overview.setAmount(booking.getAmount());
		overview.setEmployeeName(booking.getEmployee().getFirstName() + " " + booking.getEmployee().getLastName());
		overview.setStart(booking.getStart());
		overview.setEnd(booking.getEnd());
		return overview;
